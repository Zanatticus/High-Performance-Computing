...
int main(int argc, char** argv) {
	...
	for (int i = start_idx; i < end_idx; i++) {
		...
		if (bin >= NUM_BINS) {
			bin = NUM_BINS - 1;   // If the value is greater than the last bin, put it in the last
			                      // bin (if a flooring error occurs)
		}
		local_histogram[bin]++;
	}
	...
	if (rank == 0) {
		...
		std::cout << "  └── Min Time: " << std::fixed << std::setprecision(10) << min_time
		          << " seconds\n";
		std::cout << "  └── Max Time: " << std::fixed << std::setprecision(10) << max_time
		          << " seconds\n";
		std::cout << "  └── Avg Time: " << std::fixed << std::setprecision(10) << avg_time
		          << " seconds\n\n";
