...
int main(int argc, char** argv) {
	...
	if (size < COUNT_LIMIT) {
		if (rank == 0) {
			std::cout << "Error: Expected " << COUNT_LIMIT << " MPI ranks, but got " << size
			          << std::endl;
		}
		MPI_Finalize();
		return 1;
	...
	if (rank == 0) {
		counter = 1;
		std::cout << "Process " << rank << " on node " << processor_name
		          << " started with counter: " << counter << "\n";
		counter++;
	...
	if (rank == COUNT_LIMIT - 1) {
		...
		if (counter >= 0) {
			...
			// This needs to continue to finish the program since the lower ranks will hang
			MPI_Send(&counter, 1, MPI_INT, rank - 1, 0, MPI_COMM_WORLD);
		}
	}
