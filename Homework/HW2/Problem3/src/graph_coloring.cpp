    ...
    while (!vertex_list.empty()) {
        ...
        #pragma omp parallel
        {
            ...
            for (size_t i = 0; i < vertex_list.size(); i++) {
                ...
                if (node >= priorities.size()) continue;

                bool highest_priority = true;

                if (graph.find(node) != graph.end()) { // Prevents segfault from out-of-bounds
                    for (int neighbor : graph.at(node)) {
                        if (colors.find(neighbor) == colors.end() && priorities[neighbor] > priorities[node]) {
            ...
            #pragma omp critical
            independent_set.insert(independent_set.end(), local_independent_set.begin(), local_independent_set.end());
        }
