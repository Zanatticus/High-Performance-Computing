...
// Each philosopher is a thread, and the forks are the synchronizations (mutexes) needed between
// them. Philosophers think for a random amount of time and eat for a random amount of time. They
// are 'HUNGRY' if waiting for forks. Author: Zander Ingare
...
class DiningPhilosophers {
	...
	// Create the philosopher threads and start the dining process. Terminates when all threads are
	// done
	void start_dining() {
		update_display();
		for (int i = 0; i < num_philosophers; ++i) {
