...
__global__ void computeDistancesKernel(float* trainImages, float* testImage, float* distances, 
                                      int numTrainImages, int imageSize) {
...
__global__ void findMajorityLabelKernel(unsigned char* trainLabels, int* indices, 
                                      unsigned char* predictedLabel, int k) {
...
// Constructor
KNNClassifier::KNNClassifier(int k, int deviceId) : k(k), deviceId(deviceId), 
...
void KNNClassifier::freeDeviceMemory() {
    if (d_trainImages) cudaFree(d_trainImages);
    if (d_trainLabels) cudaFree(d_trainLabels);
    if (d_testImage) cudaFree(d_testImage);
    if (d_distances) cudaFree(d_distances);
    if (d_indices) cudaFree(d_indices);
    if (d_predictedLabel) cudaFree(d_predictedLabel);
...
void KNNClassifier::trainMNIST(const std::vector<float>& trainImages, 
    ...
    cudaMemcpy(d_trainImages, trainImages.data(), numTrainImages * imageSize * sizeof(float), 
              cudaMemcpyHostToDevice);
    cudaMemcpy(d_trainLabels, trainLabels.data(), numTrainImages * sizeof(unsigned char), 
              cudaMemcpyHostToDevice);
    
    std::cout << "KNN: Loaded MNIST training data with " << numTrainImages 
...
void KNNClassifier::trainCIFAR(const std::vector<float>& trainImages, 
    ...
    std::cout << "KNN: Loaded CIFAR training data with " << numTrainImages 
              << " images of size " << imageSize << std::endl;
}
...
void KNNClassifier::computeDistances(int numTestImages) {
    ...
    if (cudaStatus != cudaSuccess) {
        std::cerr << "computeDistancesKernel launch failed: " 
...
void KNNClassifier::sortDistancesAndFindMajority() {
    ...
    thrust::sort_by_key(thrust::device, 
                     thrust_distances, 
                     thrust_distances + numTrainImages, 
                     d_idx.begin());
    ...
    cudaError_t cudaStatus = cudaGetLastError();
    if (cudaStatus != cudaSuccess) {
        std::cerr << "findMajorityLabelKernel launch failed: " 
                  << cudaGetErrorString(cudaStatus) << std::endl;
        exit(1);
    }
}
