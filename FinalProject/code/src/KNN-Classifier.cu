...
__global__ void computeDistancesBatchedKernel(
    float* trainImages, float* testImages, float* allDistances, int numTrainImages, int numTestImages, int imageSize) {
...
__global__ void findMajorityLabelBatchedKernel(
    unsigned char* trainLabels, int* kIndices, unsigned char* predictions, int numTestImages, int k_neighbors) {
	int testIdx = blockIdx.x;
	int tid     = threadIdx.x;

	if (testIdx >= numTestImages)
		return;
...
void KNNClassifier::trainBatched() {
	...
	cudaMemcpy(d_testImages, h_testImages.data(), sizeof(float) * h_testImages.size(), cudaMemcpyHostToDevice);

	std::cout << "KNN Batched: Loaded " << datasetName << " data with " << numTrainImages << " training images and "
...
void KNNClassifier::computeDistances() {
	...
	computeDistancesKernel<<<gridSize, blockSize>>>(d_trainImages, d_testImage, d_distances, numTrainImages, imageSize);
}
...
float KNNClassifier::evaluateDatasetBatched() {
	...
	cudaError_t err =
	    cudaFuncSetAttribute(computeDistancesBatchedKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, sharedMemSize);
	...
	computeDistancesBatchedKernel<<<gridSize, blockSize, sharedMemSize>>>(
	    d_trainImages, d_testImages, d_distances, numTrainImages, numTestImages, imageSize);
	...
	cudaMemcpy(h_allDistances, d_distances, numTestImages * numTrainImages * sizeof(float), cudaMemcpyDeviceToHost);
	...
	for (int testIdx = 0; testIdx < numTestImages; testIdx++) {
		...
		std::partial_sort(distanceIndices.begin(), distanceIndices.begin() + k_neighbors, distanceIndices.end());
	...
	cudaMemcpy(d_indices, h_kIndices, numTestImages * k_neighbors * sizeof(int), cudaMemcpyHostToDevice);

	// Step 3: Find majority label for each test image
	findMajorityLabelBatchedKernel<<<numTestImages, blockSize>>>(
	    d_trainLabels, d_indices, d_predictedLabel, numTestImages, k_neighbors);
	...
	cudaMemcpy(h_predictions, d_predictedLabel, numTestImages * sizeof(unsigned char), cudaMemcpyDeviceToHost);
